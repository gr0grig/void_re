// gscript: tree_exit.bin
// @version: 0
// @globals: 0
// @func_table: 3 groups offsets=12,195,457
// @ft_group 0: parent=-1 stack=0 locals=0 vtable=[] imports=[(0,0)]
//   export "getAllowedTypes" args=1 cb=-1 {func_11} types=[int]
//   export "getHunterGlotokList" args=0 cb=-1 {func_12}
//   export "needWheel" args=0 cb=-1 {func_13}
//   export "getUseDistance" args=0 cb=-1 {func_14}
//   export "getMaxLimfaAmount" args=0 cb=-1 {func_15}
// @ft_group 1: parent=0 stack=0 locals=0 vtable=[] imports=[(1,0)]
//   export "onGesture" args=3 cb=-1 {func_2} types=[int,int,int]
//   export "onUse" args=3 cb=-1 {func_8} types=[str,int,int]
//   export "isPaintable" args=0 cb=-1 {func_9}
//   export "getAllowedTypes" args=1 cb=-1 {func_11} types=[int]
//   export "getHunterGlotokList" args=0 cb=-1 {func_12}
//   export "needWheel" args=0 cb=-1 {func_13}
//   export "getUseDistance" args=0 cb=-1 {func_14}
//   export "getMaxLimfaAmount" args=0 cb=-1 {func_15}
// @ft_group 2: parent=0 stack=0 locals=0 vtable=[] imports=[(2,0)]
//   export "getAllowedTypes" args=1 cb=-1 {func_11} types=[int]
//   export "getHunterGlotokList" args=0 cb=-1 {func_12}
//   export "needWheel" args=0 cb=-1 {func_13}
//   export "getUseDistance" args=0 cb=-1 {func_14}
//   export "getMaxLimfaAmount" args=0 cb=-1 {func_15}
// #export {func_2} name="onGesture"
// #export {func_8} name="onUse"
// #export {func_9} name="isPaintable"
// #export {func_11} name="getAllowedTypes"
// #export {func_12} name="getHunterGlotokList"
// #export {func_13} name="needWheel"
// #export {func_14} name="getUseDistance"
// #export {func_15} name="getMaxLimfaAmount"

// .init:-1 (locals=0)
getAllowedTypes()
{
    CallNat(r0, 20, 0x0);
}

// tree_exit.sc:21 (locals=2)
func_1()
{
    // tree_exit.sc:18
    Free1(r1);  // @src tree_exit.sc:18
    RetV(r0);
    Free1(r0);
    // tree_exit.sc:20
    CallNat(r1, 1360, 0x0);  // @src tree_exit.sc:20
}

// tree_exit.sc:55 (locals=6)
onUse()
{
    // tree_exit.sc:37
    r0 = r_neg6;  // @src tree_exit.sc:37
    r1 = -1;
    r0 = r0 == r1;
    if (!r0) goto L_0070;
    // tree_exit.sc:38
    r0 = 0;  // @src tree_exit.sc:38
    r0 = (float)r0;
    r_neg7 = r0;
    return r0;
    // tree_exit.sc:40
  L_0070:
    r1 = GetDotStr("getGestureName");  // @src tree_exit.sc:40
    r2 = "player";
    r3 = r_neg6;
    GetDot(r0, 2);
    Free2(r1, r2);
    r0 = (str)r0;
    // tree_exit.sc:42
    r1 = false;  // @src tree_exit.sc:42
    r2 = false;
    r3 = r0;
    r4 = "gesture_breach";
    r3 = r3 == r4;
    if (!r3) goto L_00fc;
    r4 = 5;
    r5 = GetDotStr("World");
    r5 = (str)r5;
    Call(r6, 0x01bc);
    if (!r3) goto L_00fc;
    r2 = true;
  L_00fc:
    if (!r2) goto L_0128;
    r3 = GetDotStr("World");
    r3 = (str)r3;
    Call(r4, 0x0264);
    if (!r2) goto L_0128;
    r1 = true;
  L_0128:
    if (!r1) goto L_0178;
    // tree_exit.sc:45
    r3 = GetDotStr("Scene");  // @src tree_exit.sc:45
    SetDotRaw(r2, 67);
    Free1(r3);
    r3 = "addExitLimfa";
    r4 = r_neg5;
    r5 = r_neg4;
    GetDot(r1, 3);
    Free3(r2, r3, r1);
    // tree_exit.sc:42
    goto L_0194;  // @src tree_exit.sc:42
    // tree_exit.sc:48
  L_0178:
    r1 = 0;  // @src tree_exit.sc:48
    r1 = (float)r1;
    r_neg7 = r1;
    Free1(r0);
    return r0;
    // tree_exit.sc:51
  L_0194:
    CallNat2(r2, 1236, 0x100);  // @src tree_exit.sc:51
    // tree_exit.sc:54
    r1 = 1;  // @src tree_exit.sc:54
    r1 = (float)r1;
    r_neg7 = r1;
    Free1(r0);
    return r0;
}

// gameplay_actions.sci:8 (locals=6)
func_3()
{
    // gameplay_actions.sci:5
    r2 = r_neg4;  // @src gameplay_actions.sci:5
    SetDotRaw(r1, 67);
    Free1(r2);
    r2 = "getPlayerEntity";
    GetDot(r0, 1);
    Free2(r1, r2);
    r0 = (str)r0;
    // gameplay_actions.sci:6
    r2 = r0;  // @src gameplay_actions.sci:6
    SetDotRaw(r1, 126);
    Free1(r2);
    r1 = (str)r1;
    // gameplay_actions.sci:7
    r5 = r1;  // @src gameplay_actions.sci:7
    SetDotRaw(r4, 137);
    Free1(r5);
    r5 = r_neg5;
    r5 = (as_string)r5;
    SetDot(r3, 1);
    Free1(r5);
    r4 = 3;
    SetDot(r2, 1);
    r2 = (bool)r2;
    r_neg6 = r2;
    Free3(r1, r0, r_neg4);
    return r0;
}

// gameplay.sci:794 (locals=4)
func_4()
{
    // gameplay.sci:788
    r1 = r_neg4;  // @src gameplay.sci:788
    Call(r2, 0x02d8);
    r2 = r_neg4;
    Call(r3, 0x03e4);
    r0 = r0 + r1;
    r0 = (float)r0;
    // gameplay.sci:789
    r1 = 2800000;  // @src gameplay.sci:789
    r1 = (float)r1;
    // gameplay.sci:791
    r2 = r0;  // @src gameplay.sci:791
    r3 = r1;
    r2 = r2 / r3;
    r3 = 0.9700000286102295f;
    r2 = r2 >= r3;
    // gameplay.sci:793
    r3 = r2;  // @src gameplay.sci:793
    r_neg5 = r3;
    Free1(r_neg4);
    return r0;
}

// gameplay.sci:781 (locals=9)
func_5()
{
    // gameplay.sci:773
    r2 = r_neg4;  // @src gameplay.sci:773
    SetDotRaw(r1, 67);
    Free1(r2);
    r2 = "getPlayerEntity";
    GetDot(r0, 1);
    Free2(r1, r2);
    r0 = (str)r0;
    // gameplay.sci:774
    r2 = r0;  // @src gameplay.sci:774
    SetDotRaw(r1, 126);
    Free1(r2);
    r1 = (str)r1;
    // gameplay.sci:776
    r2 = 0;  // @src gameplay.sci:776
    // gameplay.sci:777
    r3 = 0;  // @src gameplay.sci:777
  L_033c:
    r4 = r3;  // @src gameplay.sci:777
    r5 = 21;
    r4 = r4 < r5;
    if (!r4) goto L_03c8;
    // gameplay.sci:778
    r4 = r2;  // @src gameplay.sci:778
    r8 = r1;
    SetDotRaw(r7, 137);
    Free1(r8);
    r8 = r3;
    r8 = (as_string)r8;
    SetDot(r6, 1);
    Free1(r8);
    r7 = 0;
    SetDot(r5, 1);
    r4 = r4 + r5;
    r4 = (int)r4;
    r2 = r4;
    // gameplay.sci:777
    r4 = r3;  // @src gameplay.sci:777
    r4 = Incr(r4);
    r3 = r4;
    goto L_033c;
    // gameplay.sci:780
  L_03c8:
    r3 = r2;  // @src gameplay.sci:780
    r_neg5 = r3;
    Free3(r1, r0, r_neg4);
    return r0;
}

// gameplay.sci:730 (locals=7)
func_6()
{
    // gameplay.sci:726
    r0 = 0;  // @src gameplay.sci:726
    // gameplay.sci:727
    r4 = r_neg4;  // @src gameplay.sci:727
    SetDotRaw(r3, 67);
    Free1(r4);
    r4 = "getPlayerEntity";
    GetDot(r2, 1);
    Free2(r3, r4);
    SetDotRaw(r1, 126);
    Free1(r2);
    r1 = (str)r1;
    // gameplay.sci:728
    r2 = 0;  // @src gameplay.sci:728
  L_043c:
    r3 = r2;  // @src gameplay.sci:728
    r4 = 7;
    r3 = r3 < r4;
    if (!r3) goto L_04b8;
    r3 = r0;  // @src gameplay.sci:728
    r6 = r1;
    SetDotRaw(r5, 147);
    Free1(r6);
    r6 = r2;
    r6 = (as_string)r6;
    SetDot(r4, 1);
    Free1(r6);
    r3 = r3 + r4;
    r3 = (int)r3;
    r0 = r3;
    r3 = r2;  // @src gameplay.sci:728
    r3 = Incr(r3);
    r2 = r3;
    goto L_043c;
    // gameplay.sci:729
  L_04b8:
    r2 = r0;  // @src gameplay.sci:729
    r_neg5 = r2;
    Free2(r1, r_neg4);
    return r0;
}

// tree_exit.sc:100 (locals=0)
func_7()
{
    // tree_exit.sc:100
    return r0;  // @src tree_exit.sc:100
}

// tree_exit.sc:60 (locals=5)
func_8()
{
    // tree_exit.sc:59
    r2 = GetDotStr("Scene");  // @src tree_exit.sc:59
    SetDotRaw(r1, 67);
    Free1(r2);
    r2 = "activateTree";
    r3 = GetDotStr("self");
    r4 = 1;
    GetDot(r0, 3);
    Free4(r1, r2, r3, r0);
    // tree_exit.sc:60
    Free1(r_neg6);  // @src tree_exit.sc:60
    return r0;
}

// tree_exit.sc:65 (locals=1)
isPaintable()
{
    // tree_exit.sc:64
    r0 = true;  // @src tree_exit.sc:64
    r_neg4 = r0;
    return r0;
}

// tree_exit.sc:33 (locals=0)
getAllowedTypes()
{
    // tree_exit.sc:33
    return r0;  // @src tree_exit.sc:33
}

// gameplay.sci:595 (locals=5)
func_11()
{
    // gameplay.sci:569
    r1 = GetDotStr("!vector");  // @src gameplay.sci:569
    GetDot(r0, 0);
    Free1(r1);
    r0 = (str)r0;
    // gameplay.sci:572
    r1 = r_neg4;  // @src gameplay.sci:572
    r2 = 0;
    r1 = r1 >= r2;
    if (!r1) goto L_05c4;
    // gameplay.sci:573
    r3 = r0;  // @src gameplay.sci:573
    SetDotRaw(r2, 194);
    Free1(r3);
    r3 = 0;
    GetDot(r1, 1);
    Free2(r2, r1);
    // gameplay.sci:577
  L_05c4:
    r1 = r_neg4;  // @src gameplay.sci:577
    r2 = 172800;
    r1 = r1 >= r2;
    if (!r1) goto L_0650;
    // gameplay.sci:578
    r4 = GetDotStr("World");  // @src gameplay.sci:578
    SetDotRaw(r3, 198);
    Free1(r4);
    SetDotRaw(r2, 203);
    Free1(r3);
    r3 = "tutorial_quest_kill_predators_done";
    GetDot(r1, 1);
    Free2(r2, r3);
    if (!r1) goto L_0650;
    // gameplay.sci:579
    r3 = r0;  // @src gameplay.sci:579
    SetDotRaw(r2, 194);
    Free1(r3);
    r3 = 1;
    GetDot(r1, 1);
    Free2(r2, r1);
    // gameplay.sci:584
  L_0650:
    r1 = r_neg4;  // @src gameplay.sci:584
    r2 = 259200;
    r1 = r1 >= r2;
    if (!r1) goto L_0698;
    // gameplay.sci:585
    r3 = r0;  // @src gameplay.sci:585
    SetDotRaw(r2, 194);
    Free1(r3);
    r3 = 2;
    GetDot(r1, 1);
    Free2(r2, r1);
    // gameplay.sci:590
  L_0698:
    r1 = r_neg4;  // @src gameplay.sci:590
    r2 = 864000.0f;
    r1 = r1 > r2;
    if (!r1) goto L_06e0;
    r3 = r0;  // @src gameplay.sci:590
    SetDotRaw(r2, 194);
    Free1(r3);
    r3 = 3;
    GetDot(r1, 1);
    Free2(r2, r1);
    // gameplay.sci:594
  L_06e0:
    r1 = r0;  // @src gameplay.sci:594
    r_neg5 = r1;
    Free2(r1, r0);
    return r0;
}

// gameplay.sci:877 (locals=4)
getHunterGlotokList()
{
    // gameplay.sci:864
    r1 = GetDotStr("!vector");  // @src gameplay.sci:864
    GetDot(r0, 0);
    Free1(r1);
    r0 = (str)r0;
    // gameplay.sci:866
    r3 = r0;  // @src gameplay.sci:866
    SetDotRaw(r2, 194);
    Free1(r3);
    r3 = 8;
    GetDot(r1, 1);
    Free2(r2, r1);
    // gameplay.sci:867
    r3 = r0;  // @src gameplay.sci:867
    SetDotRaw(r2, 194);
    Free1(r3);
    r3 = 13;
    GetDot(r1, 1);
    Free2(r2, r1);
    // gameplay.sci:868
    r3 = r0;  // @src gameplay.sci:868
    SetDotRaw(r2, 194);
    Free1(r3);
    r3 = 14;
    GetDot(r1, 1);
    Free2(r2, r1);
    // gameplay.sci:869
    r3 = r0;  // @src gameplay.sci:869
    SetDotRaw(r2, 194);
    Free1(r3);
    r3 = 20;
    GetDot(r1, 1);
    Free2(r2, r1);
    // gameplay.sci:872
    r3 = r0;  // @src gameplay.sci:872
    SetDotRaw(r2, 194);
    Free1(r3);
    r3 = 1;
    GetDot(r1, 1);
    Free2(r2, r1);
    // gameplay.sci:876
    r1 = r0;  // @src gameplay.sci:876
    r_neg4 = r1;
    Free2(r1, r0);
    return r0;
}

// tree_exit.sc:9 (locals=1)
needWheel()
{
    // tree_exit.sc:8
    r0 = false;  // @src tree_exit.sc:8
    r_neg4 = r0;
    return r0;
}

// tree_exit.sc:14 (locals=1)
getUseDistance()
{
    // tree_exit.sc:13
    r0 = 3;  // @src tree_exit.sc:13
    r0 = (float)r0;
    r_neg4 = r0;
    return r0;
}

// tree_exit.sc:26 (locals=1)
getMaxLimfaAmount()
{
    // tree_exit.sc:25
    r0 = 10000;  // @src tree_exit.sc:25
    r_neg4 = r0;
    return r0;
}

